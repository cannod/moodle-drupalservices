...
class auth_plugin_drupalservices extends auth_plugin_base {
    ...
    function get_drupal_session($cfg = null) {
        if (!$cfg) {
            ...
            debugging("<pre>loaded saved session settings config:" .
                print_r(array('host' => $cfg->host_uri, 'cookie domain' => $cfg->cookiedomain), true) . "</pre>", DEBUG_DEVELOPER);
        }
    ...
    function detect_sso_settings($cookiebydomain) {
        ...
        do {
            ...
            } while ($iloopbreak < 100 && !$sso_config_discovered &&
            $cookiebypath = auth_plugin_drupalservices::dereference_url($cookiebypath, false));
            //loop again until there is only one item left in the domain part of the url
        } while ($iloopbreak < 100 && !$sso_config_discovered &&
        $cookiebydomain = auth_plugin_drupalservices::dereference_url($cookiebydomain, true));
    ...
    function create_update_user($drupal_user) {
            ...
            foreach ($this->userfields as $field) {
                if (isset($this->config->{"field_map_$field"})) {
                    ...
                    if (!empty($drupalfield)) {
                        ...
                        } else if (!is_array($drupal_user->$drupalfield)) {
        ...
        if (empty($user->country)) {
            $user->country = "none";
        } // this is too big just to make a point
    ...
    function sync_users($do_updates = false) {
        ...
        foreach ($drupal_users as $drupal_user_info) {
            ...
            print_string('auth_drupalservicesupdateuser', 'auth_drupalservices',
                array($drupal_user->name . '(' . $drupal_user->uid . ')'));
    ...
    function moodle_user_cohorts($user) {
        global $DB;
        $sql =
            "SELECT c.id AS cid, c.name AS name FROM {cohort} c JOIN {cohort_members} cm ON cm.cohortid = c.id WHERE c.component = 'auth_drupalservices' AND cm.userid = $user->id";
